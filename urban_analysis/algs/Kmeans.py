...
from urban_analysis.forms.KmeansWssParam import ParameterWss
import numpy as np
...
class Kmeans(QgisAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        wss_param = ParameterWss(self.WSS, self.tr(u'군집 수 변화에 따른 WSS 변화패턴'), layer_param=self.INPUT, max_param=self.MAX_K)
        wss_param.setMetadata({'widget_wrapper': {'class': 'urban_analysis.forms.KmeansWss.WssWidgetWrapper'}})
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if nCluster > len(pts):
            feedback.pushInfo(self.tr(u'군집의 수가 데이터 개수 보다 더 많습니다.<br> 데이터 개수 만큼만 군집 수가 나뉘어 집니다.'))
            nCluster = len(pts)
