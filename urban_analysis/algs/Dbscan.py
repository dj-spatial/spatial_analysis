...
from urban_analysis.forms.DbscanKnnParam import ParameterKnn
import processing
...
class Dbscan(QgisAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterNumber(self.EPSILON,
                                                       self.tr(u'군집 내 포인트 간 최대거리(ε)'),
                                                       QgsProcessingParameterNumber.Double,
                                                       0.1, False, 0, 99999999))
        self.addParameter(QgsProcessingParameterNumber(self.MINPOINTS,
                                                       self.tr(u'최소군집크기(minPts)'),
                                                       QgsProcessingParameterNumber.Integer,
                                                       3, False, 2, 99999999))
        knn_param = ParameterKnn(self.KNN, self.tr(u'K개 최근린 거리분포'), layer_param=self.INPUT, k_param=self.MINPOINTS)
        knn_param.setMetadata({'widget_wrapper': {'class': 'urban_analysis.forms.DbscanKnn.KnnWidgetWrapper'}})
