...
class HarchiWidget(BASE, WIDGET):
    ...
    def getHarchi(self):
        ...
        if self.comboBox.currentIndex() == 0:
            ...
            dHover = [u'마지막' + str(n) + u'번째 군집' + '<br>' + 'Cophenet 거리 = ' + '{0:,.1f}'.format(d) for n, d in zip(x_range, revDistance)]
            diffHover = [u'마지막' + str(n) + u'번째 군집' + '<br>' + '거리감소 = ' + '{0:,.1f}'.format(d) for n, d in zip(x_range, revDiff)]
    ...
    def plotView(self):
        msg = self.getHarchi()
        if  msg != 'Success':
            QMessageBox.information(self, u"입력 값 오류", msg)
        else:
